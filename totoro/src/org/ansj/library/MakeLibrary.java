	...
	public static void sortLibrary(BufferedReader reader,int status2,int status3) throws IOException {
		...
		while ((line = reader.readLine()) != null) {
			...
			for (int i = 1; i < length; i++) {
				...
				if ((strTemp = treeMap.get(tempStr)) != null) {
					if (i == length - 1) {
						if (strTemp.getStatu() == 1) {
							if(strTemp.getNature()==null){
								strTemp.setWeight(Integer.parseInt(temps[2])) ;
								strTemp.setNature(temps[0]) ;	
							}
				...
				if(str.getStatu()>1){
					str.setWeight(Integer.parseInt(temps[2])) ;
					str.setNature(temps[0]) ;
				}
